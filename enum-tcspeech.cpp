...
void GettcspeechAudioDevices_(vector<AudioDeviceInfo> &devices, bool input)
{
...
void GettcspeechAudioDevices(vector<AudioDeviceInfo> &devices, bool input)
{
	devices.clear();

	try {
		GettcspeechAudioDevices_(devices, input);

	} catch (HRError error) {
		blog(LOG_WARNING, "[GettcspeechAudioDevices] %s: %lX",
				error.str, error.hr);
	}
}
